...
RSpec.describe StoreModel::Types::One do
  ...
  describe "#type" do
    ...
    context "when storage is json" do
      let(:storage) { :json }

      it { is_expected.to eq(:json) }
    end

    context "when storage is hstore" do
      let(:storage) { :hstore }

      it { is_expected.to eq(:hstore) }
    end
  end
  ...
  describe "#cast_value" do
    ...
    shared_examples "qwe" do
      ...
      context "when some keys are not defined as attributes" do
        ...
        context "when String is passed" do
          let(:value) { encoder.call(attributes) }
          include_examples "for unknown attributes"
        end
    ...
    context "when storage is json" do
      let(:storage) { :json }
      let(:encoder) { ->(attributes) { ActiveSupport::JSON.encode(attributes) } }

      include_examples "qwe"
    end

    context "when storage is hstore" do
      let(:storage) { :hstore }
      let(:encoder) { ->(attributes) { ActiveRecord::ConnectionAdapters::PostgreSQL::OID::Hstore.new.serialize(attributes) } }
